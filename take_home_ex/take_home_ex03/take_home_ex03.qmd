...
5.  I will repeat the same step to download the data for **December** of the year **1983, 1993, 2003, and 2013** in **Clementi**
...
Colour should be different from the background.
...
gg_plot <- ggplot(combined_rainfall_1, aes(x = Date, y = Rainfall)) +
  geom_point() +
  labs(title = 'Daily Rainfall in Clementi - 1983 to 2023 (Selected Period)', x = 'Year', y = 'Daily Rainfall (mm)') +
  theme_minimal()+
...
Based on the interactive visualisation above, using only Dec in 1983, 1993, 2003, 2013, and 2023, the claims made by NCCS seem to be invalid. In 2003, the rainfalls seem to be have the widest spread. Average rainfall did not rise as drastically as mentioned above.

Further studies can be done by analysing all the data.
